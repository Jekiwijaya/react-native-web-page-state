...
module.exports = {
  getBlacklistRE() {
    const appFolderRegex = new RegExp(`react-native-web-page-state/examples/(?!${appdata.name})(.*)`);
    console.log(appFolderRegex);
    return blacklist([
      /react-native-web-page-state\/node_modules\/react-native\/(.*)/,
      appFolderRegex,
      /react-native-web-page-state\/node_modules\/react\/(.*)/,
    ]);
  },
